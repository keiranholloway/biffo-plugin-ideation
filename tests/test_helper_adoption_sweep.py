...
`class:drift` (biffo-plugin-marketing#119): a shared helper is introduced to fix
a defect, adopted at some call sites and not others, and the helper's existence
makes everyone believe it is fixed. The unconverted sites keep the bug, behind a
closed issue, looking done.

The estate has three recorded instances, all the same shape. Marketing's
`public_base_url_for` was adopted at **1 call site of 3** — the two missed sites
500'd both packs (marketing#72). Its artefact-body parser was filed against
**2** sites when the tree actually held **7 across 6 files** — 3.5x the reported
blast radius, and the five nobody named would have survived a fix made as filed
(marketing#49/#111). And **this repo was instance three**: `adapter.py` held
``_load_json_column`` while `service.py` wrote the same normalisation out by
hand, which is the change this guard ships with.

## Why this shape of guard

The obvious guard is a grep for the one ternary. It would have caught instance
three and nothing else, because it names a literal — and **nothing prompts an
author to write the next one**, so a helper introduced next week is unguarded by
construction and the first anyone hears of it is a defect in production. That is
the residual gap the class issue describes.

So this guard **derives its own subject list from the source tree**: it finds
the plugin's consolidation helpers by shape, then asks whether any expression
elsewhere in the package re-derives one. A helper written after this file is in
the sweep the moment it is written, with nothing to remember and nothing to
update.

## What counts as a consolidation helper

A function whose entire body is ``return <expression>`` (a docstring is
allowed) — the shape of every helper this class has produced: one expression,
held once, called from many places. That expression becomes a **template** whose
parameters are wildcards, and any expression that structurally unifies with it
is a call site that bypassed the helper.
...
- **FastAPI dependency providers** (a parameter defaulting to ``Depends(...)``).
  Routers declare their own on purpose — `dependency_overrides` is keyed by the
  function object, so a shared provider could not be overridden per app.
- **Pure delegations** — a lone call forwarding its parameters unchanged, e.g.
  ``return await self._load_owned(owner_sub=owner_sub, session_id=session_id)``.
  A façade holds no expression, so every ordinary call to `_load_owned` would
  otherwise be reported as bypassing `get_session`.
- **Expressions below `_MIN_TEMPLATE_NODES`.** ``return value or {}`` matches
  half the plugin and is evidence of nothing.

## What it deliberately does NOT catch

This finds *duplicated expressions*. The other half of the class — a call to a
**superseded function** the newer helper replaced (marketing#72's shape:
`public_base_url` still called where `public_base_url_for` was needed) — leaves
no duplicated expression behind, and nothing in the source records that one
function supersedes another. Anyone reading this file as "the drift class is now
impossible" should read this paragraph again: it closes one shape of the class.

## Waivers are a ledger, not a mute button

A real duplicate that should *not* be consolidated goes in `_ACCEPTED_DUPLICATES`
with its reason, so it is one review-visible line rather than a silently
...
_SRC = Path(__file__).resolve().parents[1] / "src" / "ideation"

#: Below this many AST nodes an expression is too common to mean anything.
#: `parse_json_text`'s ternary is 20 nodes; 8 admits it with room to spare and
#: still rejects `value or {}` (4).
...
#: green over an empty denominator, which is precisely the failure mode this
#: class is about: a guard nobody notices is covering nothing.
#: `parse_json_text` is the helper whose bypassed call site was this repo's own
#: instance, so if it stops being enumerated the sweep has stopped working.
_MUST_ENUMERATE = {"parse_json_text"}

#: (helper name, file that re-derives it) -> why that copy is correct.
#: Empty, and it should be argued about before it stops being empty: every
#: entry is a copy this plugin has decided to keep.
...
def _sole_returned_expression(fn: ast.FunctionDef | ast.AsyncFunctionDef) -> ast.expr | None:
    ...
    A leading docstring is skipped, because every helper in this plugin has one
    and requiring a bare body would exclude all of them.
    """
    body = list(fn.body)
    if (
        body
...
#: estate's most expensive drift shape — marketing#72 was a base URL — and an
#: f-string helper contains no call at all; leaving them out silently excluded
#: that whole family from the sweep, which a planted duplicate caught before
#: this guard shipped.
_COMPOSING_NODES = (
    ast.Call,
    ast.IfExp,
...
def consolidation_helpers(trees: dict[str, ast.AST]) -> list[_Helper]:
    ...
    This is the sweep's **denominator**, and the reason the guard needs no
    hand-maintained list of helper names: a helper added tomorrow lands here on
    its own.
    """
    found: list[_Helper] = []
...
def _unifies(
    ...
    ``json.loads(value) if isinstance(value, str) else value`` matches a call
    site using one variable throughout and not one that mixes two.
    """
...
def test_no_module_re_derives_a_shared_helpers_expression() -> None:
    ...
    assert not offenders, (
        ...
        "instead of calling it. A hand-written copy stops tracking the helper "
        "the moment the helper is corrected, and review cannot see the "
...
def test_the_sweep_actually_enumerates_this_plugins_helpers() -> None:
    ...
    The whole point of this class is that nobody notices a guard covering
    nothing, so the denominator is asserted rather than assumed: it must be
    non-trivial, and it must contain the helper whose bypassed call site was
    this repo's own instance.
    """
    helpers = consolidation_helpers(_parse_package())
    names = {h.name for h in helpers}
    assert len(helpers) >= 5, f"suspiciously few helpers enumerated: {sorted(names)}"
...
def test_the_guard_can_actually_fail() -> None:
    ...
    Deliberately *not* the call site this change converted: proving a guard
    against the code that motivated it only proves it stays green. This plants a
    fresh helper and a site that bypasses it.
    """
    tree = ast.parse(
        "def _normalise(raw):\n"
        '    """Hold this once."""\n'
...
def test_a_near_miss_is_not_reported() -> None:
    ...
    (Each near-miss carries a second statement so it is a call site rather than
    a helper of its own. A one-line function IS a template, and a template whose
    parameters are wildcards legitimately matches another helper's body — that
    is a duplicated *helper*, which this guard reports on purpose.)
    """
    tree = ast.parse(
        "def _normalise(raw):\n"
        "    return json.loads(raw) if isinstance(raw, str) else (raw or {})\n"
...
def test_a_dependency_provider_is_not_treated_as_a_helper() -> None:
    """Routers declare identical `Depends` providers on purpose.

    `dependency_overrides` is keyed by the function object, so a shared provider
    could not be overridden for one app under test. Without this exclusion the
    sweep would open with a wall of offenders whose only correct resolution is a
    waiver — and a guard that starts life full of waivers is one nobody reads.
    """
    tree = ast.parse(
        "def get_service(user = Depends(require_user)):\n"
        "    return IdeationService(CoreHttpGateway(transport), min_turns=3)\n"
        "\n"
        "def other_router_provider(user = Depends(require_user)):\n"
        "    return IdeationService(CoreHttpGateway(transport), min_turns=3)\n"
    )
...
def test_a_pure_delegation_is_not_treated_as_a_helper() -> None:
    """`IdeationService.get_session` is exactly this shape.

    It returns ``await self._load_owned(...)`` and nothing else, so it holds no
    expression. Were it treated as a template, the four other methods that
    legitimately call `_load_owned` at the top of their own work would each be
    reported as bypassing it — four false offenders on day one.
    """
    tree = ast.parse(
        "class S:\n"
...
def test_no_accepted_duplicate_has_gone_stale() -> None:
    ...
    Each entry must still name a duplicate the sweep really finds. When the copy
    is consolidated or the file renamed, this fails and the excuse goes with it,
    rather than sitting there granting a permission nobody re-examined.
    """
